        ...
        $(document).ready(function() {
            ...
            var intervalId = window.setInterval(() => {
                @if (request('job_status_id') != '')
                    $.getJSON(
                        "{{ route(auth()->user()->akses . '.jobstatus.show', request('job_status_id')) }}",
                        function(data, textStatus, jqXHR) {
                            ...
                            if (isEnded) {
                                window.location.href =
                                    "{{ route(auth()->user()->akses . '.jobstatus.index') }}";
                            }
                        }
                    );
                @else
        ...
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                    <div class="row my-3">
                        <div class="col-md-6">
                            <a href="{{ route(auth()->user()->akses . '.tagihan.create') }}" class="btn btn-primary mt-4">
                                Tambah Tagihan SPP
                            </a>
                                ...
                                @forelse ($models as $item)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @else
                                                <a
                                                    href="{{ route(auth()->user()->akses . '.jobstatus.index', ['job_status_id' => $item->id]) }}">
                                                    {{ $item->type }}
                                                </a>
                                            @endif
                                        </td>
