...
class TagihanLainStepController extends Controller
{
    ...
    public function step1()
    {
        ...
        return view('user.tagihanlain_step1', $data);
    }
    ...
    public function step2()
    {
        ...
        if (session('tagihan_untuk') == "" || !session()->has('tagihan_untuk')) {
            ...
            return redirect()->route(auth()->user()->akses . '.tagihanlainstep.create', ['step' => 1]);
        }

        // step 1 (semua) -> step 3
        if (session('tagihan_untuk') == 'semua' && !session()->has('step')) {
            return redirect()->route(auth()->user()->akses . '.tagihanlainstep.create', ['step' => 3]);
        }
        ...
        return view('user.tagihanlain_step2', $data);
    }
    ...
    public function step3()
    {
        ...
        if (!session()->has('data_siswa') && session('tagihan_untuk') == 'pilihan') {
            ...
            return redirect()->route(auth()->user()->akses . '.tagihanlainstep.create', ['step' => 2]);
        }

        $data['activeStep3'] = 'active';
        $data['biayaList'] = Biaya::whereNull('parent_id')->pluck("nama", "id");
        return view('user.tagihanlain_step3', $data);
    }
    ...
    public function step4()
    {
        ...
        return view('user.tagihanlain_step4', $data);
    }
}
